...
df[df["set"] == 22]["acc_x"].plot()

# Calculate time duration of a set
duration = df[df["set"] == 4].index[-1] - df[df["set"] == 4].index[0]
duration.seconds
...
# Overwrite the data using the filter
for col in predictor_columns:
...
plt.xlabel("principal componet number")
...
fs = int(1000 / 200)
ws = int(2800 / 200)  # Average time for a repetition
...
df_freq = pd.concat(df_freq_list).set_index("epoch(ns)", drop=True)
# --------------------------------------------------------------
