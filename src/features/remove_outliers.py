...
df[outlier_columns[:3] + ["label"]].boxplot(by="label", figsize=(25, 15), layout=(1, 3))
df[outlier_columns[3:] + ["label"]].boxplot(by="label", figsize=(25, 15), layout=(1, 3))
...
df[outlier_columns[:3] + ["label"]].plot.hist(
    by="label", figsize=(25, 25), layout=(3, 3)
)
df[outlier_columns[3:] + ["label"]].plot.hist(
    by="label", figsize=(25, 25), layout=(3, 3)
)
...
for col in outlier_columns:
    for label in df["label"].unique():
        ...
        outliers_remove_df.loc[(outliers_remove_df["label"] == label), col] = dataset[
            col
        ]

        n_outliers = len(dataset) - len(dataset[col].dropna())
        print(f"Removed {n_outliers} from {col} {label}")
